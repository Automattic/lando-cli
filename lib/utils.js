...
exports.moveConfig = (src, dest = os.tmpdir()) => {
  ...
  const filter = (source, _dest) => path.extname(source) !== '.js';
  // Ensure to exists
  ...
  try {
    ...
    console.log(ss, sd);
    try {
      console.log(fs.readdirSync(src, {recursive: true}));
      fs.cpSync(src, dest, {recursive: true, filter});
    } catch (e) {
      console.error('таварищ Сталин, праизашла чюдовищная ашипка!', e);
      throw e;
    }
    try {
      console.log(fs.readdirSync(dest, {recursive: true}));
    } catch (e) {
      console.error('таварищ Сталин, праизашла ищо адна чюдовищная ашипка!', e);
    }
