...
/**
 * Loads init plugin definitions from plugin directories.
 * @param {string[]} dirs Directories to scan.
 * @returns {object[]} Init plugin configs.
 */
...
/**
 * Loads init source definitions from plugin directories.
 * @param {string[]} dirs Directories to scan.
 * @returns {object[]} Init source configs.
 */
...
/**
 * Bootstraps plugin discovery and merges plugin config into Lando.
 * @param {Lando} lando Lando instance.
 * @returns {Promise} Promise for plugin bootstrap.
 */
...
/**
 * Bootstraps tasks, init sources, and cached task metadata.
 * @param {Lando} lando Lando instance.
 * @returns {Promise} Promise for task bootstrap.
 */
...
/**
 * Bootstraps shell, scan helpers, engine, scripts, and docker version data.
 * @param {Lando} lando Lando instance.
 * @returns {Promise} Promise for engine bootstrap.
 */
const bootstrapEngine = lando => {
  ...
  const versionsPromise = new Promise(resolve => {
    const versions = lando.cache.get('docker-version');
    if (!versions) {
...
/**
 * Bootstraps builders and YAML helpers for app-level operations.
 * @param {Lando} lando Lando instance.
 * @returns {void}
 */
...
/**
 * Routes bootstrap execution to the requested bootstrap level.
 * @param {'config'|'tasks'|'engine'|'app'} level Bootstrap level.
 * @param {Lando} lando Lando instance.
 * @returns {Promise|boolean|void} Bootstrap result.
 */
...
module.exports = class Lando {
  /**
   * @param {object} [options] Options to initialize the Lando instance with.
   */
  constructor(options = {}) {
    /** @type {object} */
   ...
   * @param {string} [level] Level with which to bootstrap Lando.
    * @returns {Promise<Lando>} Promise resolving to the bootstrapped Lando instance.
   * @example
   * // Bootstrap lando at default level and then exit
    * lando.bootstrap().then(() => process.exit(0))
   */
  bootstrap(level = 'app') {
   ...
   * @since 3.0.0
   * @alias lando.getApp
   * @param {string} [startFrom] The directory to start looking for an app.
   * @param {boolean} [warn] Show a warning if we can't find an app.
  * @returns {import('./app')|undefined} Instantiated App instance when found.
   * @example
